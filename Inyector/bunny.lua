...
corner(hdr, 13)
rgrad(hdr, 0)
local hdrStroke = new("UIStroke", { Parent = hdr, Color = C_HOT, Thickness = 1, Transparency = 0.25 })
task.spawn(function()
    while hdr.Parent do
        TweenService:Create(hdrStroke, TweenInfo.new(1.5, Enum.EasingStyle.Sine), { Transparency = 0.1 }):Play()
        task.wait(1.5)
        TweenService:Create(hdrStroke, TweenInfo.new(1.5, Enum.EasingStyle.Sine), { Transparency = 0.4 }):Play()
        task.wait(1.5)
    end
end)
