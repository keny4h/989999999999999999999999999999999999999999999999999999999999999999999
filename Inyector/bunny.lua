...
addStroke(loaderFrame, THEME.PRIMARY, 2)
...
addCorner(spinner, 10)

task.spawn(function()
    for i = 1, 100 do
        TweenService:Create(loaderBar, TweenInfo.new(0.02), { Size = UDim2.new(i/100, 0, 1, 0) }):Play()
        loaderText.Text = ("Loading... %d%%"):format(i)
        task.wait(0.02)
    end
    TweenService:Create(loaderFrame, TweenInfo.new(0.5, Enum.EasingStyle.Back, Enum.EasingDirection.In), {
        Size = UDim2.fromOffset(0, 0),
        Position = UDim2.new(0.5, 0, 0.5, 0),
    }):Play()
    task.delay(0.5, function() loaderGui:Destroy() end)
end)
...
local function openUI()
    ...
    win.Position = UDim2.new(0.5, -170, 0.5, -210 - 50)
