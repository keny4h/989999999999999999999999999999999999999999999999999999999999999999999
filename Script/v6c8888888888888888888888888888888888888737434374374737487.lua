...
-- ============================================================
-- NATSUKI THEME V6 — Paleta de Natsuki (rosas, blancos, tonos suaves)
-- ============================================================
local ACCENT      = Color3.fromRGB(255, 105, 180)      -- Hot pink/Natsuki pink
local ACCENT2     = Color3.fromRGB(219, 112, 147)      -- Medium violet red
local BG_DARK     = Color3.fromRGB(25, 18, 22)         -- Dark pink-tinted background
local BG_FRAME    = Color3.fromRGB(35, 25, 32)         -- Frame oscuro con tinte rosa
local BG_ELEM     = Color3.fromRGB(45, 32, 40)         -- Elemento oscuro rosado
local TEXT_WHITE  = Color3.fromRGB(255, 245, 247)      -- Soft white with pink tint
local TEXT_GRAY   = Color3.fromRGB(180, 150, 160)      -- Muted pink gray
local TEXT_MID    = Color3.fromRGB(220, 185, 195)      -- Mid pink tone
local OUTLINE     = Color3.fromRGB(80, 50, 60)         -- Pink-tinted outline
local ACCENT_GLOW = Color3.fromRGB(255, 182, 193)      -- Light pink glow
...
local function ShowSystemLoader(onFinished)
    ...
    bg.BackgroundColor3 = Color3.fromRGB(25, 18, 22)
    bg.BackgroundTransparency = 0
    bg.Parent = ScreenGui

    -- Línea de acento horizontal animada - Natsuki theme
    ...
    subtitle.TextColor3 = Color3.fromRGB(219, 112, 147)
    ...
    tweenExpo(accentLine, 0.4, {
        Size = UDim2.new(0.3, 0, 0, 2),
        Position = UDim2.new(0.35, 0, 0.5, 30)
    }):Play()
    task.wait(0.2)

    tweenBack(title, 0.45, { TextTransparency = 0 }):Play()
    task.wait(0.25)
    ...
    for _, text in ipairs(steps) do
        ...
        task.wait(0.5)
        tweenSine(subtitle, 0.2, { TextTransparency = 1 }):Play()
        task.wait(0.12)
    end

    -- Salida
    tweenExpo(accentLine, 0.25, {
        Size = UDim2.new(1, 0, 0, 2),
        Position = UDim2.new(0, 0, 0.5, 30)
    }):Play()
    task.wait(0.12)
    TweenService:Create(bg, TweenInfo.new(0.35, Enum.EasingStyle.Quint), { BackgroundTransparency = 1 }):Play()
    TweenService:Create(title, TweenInfo.new(0.25, Enum.EasingStyle.Sine), { TextTransparency = 1 }):Play()
...
local function LoadVxnityHub()
    ...
    -- ============================================================
    -- BALL CONTROL ASSIST (Dribbling Helper)
    -- ============================================================
    ...
    DribbleTab:Toggle({
        ...
        Desc = "Improve ball control consistency while dribbling",
        Callback = function(state)
    ...
    DribbleTab:Slider({
        ...
        Callback = function(val)
            ...
            dribbleIntensity = levels[math.floor(val)] or "Medium"
        end
    })

    DribbleTab:Toggle({
        Title = "Activation: Click",
        Desc = "Activate by clicking (for mobile: tap)",
        Callback = function(state)
    ...
    DribbleTab:Toggle({
        ...
        Desc = "Activate by holding the button",
        Callback = function(state)
    ...
    DribbleTab:Button({
        Title = "Test Dribble",
        Desc = "Test the dribble assist activation",
        Callback = function()
            if dribbleEnabled then
                VxnityUI:Notify({ Title = "Dribble Test", Desc = "Intensity: " .. dribbleIntensity, Duration = 1.5 })
            end
        end
    })
    ...
    local function applyDribbleAssist()
        ...
        local config = DRIBBLE_CONFIG[dribbleIntensity] or DRIBBLE_CONFIG.Medium

        pcall(function() ball.CanCollide = false end)
        pcall(function() ball:SetNetworkOwner(LocalPlayer) end)

        local targetPos = hrp.Position + hrp.CFrame.LookVector * config.offset + Vector3.new(0, 0.05, 0)
        local diff = targetPos - ball.Position
        local dist = diff.Magnitude

        if dist > config.deadzone then
            local speed = math.clamp(dist * config.strength, 100, config.strength)
            ball.AssemblyLinearVelocity = diff.Unit * speed + hrp.CFrame.LookVector * 15
        else
            ball.AssemblyLinearVelocity = ball.AssemblyLinearVelocity * 0.95 + hrp.CFrame.LookVector * 10
        end
