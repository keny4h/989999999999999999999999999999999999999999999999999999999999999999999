...
local ACCENT2 = Color3.fromRGB(0, 130, 200)
local BG_DARK = Color3.fromRGB(10, 10, 15)
local BG_FRAME = Color3.fromRGB(18, 18, 25)
local BG_ELEM = Color3.fromRGB(25, 25, 35)
local TEXT_WHITE = Color3.fromRGB(255, 255, 255)
local TEXT_GRAY = Color3.fromRGB(120, 120, 140)
local TEXT_MID = Color3.fromRGB(170, 170, 190)
local OUTLINE = Color3.fromRGB(40, 40, 55)
local ACCENT_GLOW = Color3.fromRGB(0, 200, 255)
...
local function LoadVxnityHub()
    ...
    ReactTab:Section({ Title = " SKULL" })

    ReactTab:Button({ Title = "hockurea", Desc = " . . . . ",
        Callback = function()
            currentReactPower = 5e18; P.reactPower = currentReactPower
            enableReactHook(); applyReactInstant(currentReactPower)
            XynorUI:Notify({ Title = "SKULL 2", Desc = " . . .", Duration = 2 })
        end
    })
    ReactTab:Button({ Title = "Kn4", Desc = " . . .",
        Callback = function()
            currentReactPower = 1e25; P.reactPower = currentReactPower
            enableReactHook(); applyReactInstant(currentReactPower)
            XynorUI:Notify({ Title = "Akuuroshii", Desc = " . . .", Duration = 2 })
        end
    })
    ReactTab:Button({ Title = "React", Desc = " . . . ",
        Callback = function()
            currentReactPower = 2e22; P.reactPower = currentReactPower
            enableReactHook(); applyReactInstant(currentReactPower)
            XynorUI:Notify({ Title = "HYPER", Desc = " . . . ", Duration = 2 })
        end
    })
    ReactTab:Button({ Title = "React 2", Desc = " . . . ",
        Callback = function()
            currentReactPower = 8e20; P.reactPower = currentReactPower
            enableReactHook(); applyReactInstant(currentReactPower)
            XynorUI:Notify({ Title = "ULTIMATE", Desc = " . . . ", Duration = 2 })
        end
    })
    ReactTab:Button({ Title = "React 3", Desc = " . . . ",
        Callback = function()
            currentReactPower = 1e28; P.reactPower = currentReactPower
            enableReactHook(); applyReactInstant(currentReactPower)
            XynorUI:Notify({ Title = "rEACT 4", Desc = " . . . ", Duration = 2 })
        end
    })
    ...
    ReactTab:Button({ Title = "✝️ KENYAH ", Desc = "Max power + predicion",
        Callback = function()
    ...
    HelpersTab:Section({ Title = ":33333" })
    ...
    local CONFIG = {
        FOLLOW_DISTANCE = 0.0801,
        FOLLOW_SPEED    = 300000000000000000000000000000,
        DEAD_ZONE       = 0.03,
        MAX_DISTANCE    = 0.1,
        STRONG_PULL     = 50000,
        SOFT_PULL       = 35000,
        MAGNET_PULL     = 8000000000000000000000,
        PREDICT_OFFSET  = 0.04,
    ...
    local function handleDribbleInput(input, gp)
        ...
        if dribbleActivation == "Click" then
            if input.UserInputType == Enum.UserInputType.MouseButton1 
                ...
                dribbleActive = true
                applyDribbleAssist()
                task.delay(0.01, function() dribbleActive = false end)
            end
