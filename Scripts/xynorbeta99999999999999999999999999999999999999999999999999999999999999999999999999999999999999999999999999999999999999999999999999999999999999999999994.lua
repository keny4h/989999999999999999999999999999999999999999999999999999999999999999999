--[[
    ═══════════════════════════════════════════════════════════════
                    XYNOR HUB 3.0 - HIIII SKIDDDDDDDDDDDDDDDDDDD
...
local function CreateLoader()
    ...
    Container.AnchorPoint = UDim2.new(0.5, 0, 0.5, 0)
...
local function BuildMainWindow()
    ...
    function WindowAPI:Tab(section, opts)
        ...
        btn.MouseButton1Click:Connect(function()
            ...
            if Pages[tab.Title] then
                task.spawn(function()
                    task.wait(0.1)
                    Pages[tab.Title].Visible = true
                end)
            end
            CurrentPage = btn
        end)
...
function runMainHub()
    ...
    local Home = WindowAPI:CreateElement(HomeTab.Page)
    ...
    local Reach = WindowAPI:CreateElement(ReachTab.Page)
    ...
    local Moss = WindowAPI:CreateElement(MossingTab.Page)
    ...
    local React = WindowAPI:CreateElement(ReactTab.Page)
    ...
    local Help = WindowAPI:CreateElement(HelpTab.Page)
    ...
    local Aim = WindowAPI:CreateElement(AimTab.Page)
    ...
    local Opt = WindowAPI:CreateElement(OptTab.Page)
    ...
    local Dribble = WindowAPI:CreateElement(DribbleTab.Page)
...
task.spawn(function()
    for i = 1, 100, 2 do
        if not loader.Gui.Parent then return end
        task.wait(0.035)
        loader.ProgressBar.Size = UDim2.new(i/100, 0, 1, 0)
        loader.StatusText.Text = "Loading modules... "..i.."%"
    end
